...
import com.example.autoshkolla.databinding.ItemSignsBinding
import com.example.autoshkolla.model.SignModel
import com.squareup.picasso.Picasso

class CategoryASignsAdapter : RecyclerView.Adapter<CategoryASignsAdapter.SignDataViewHolder>() {
    ...
    inner class SignDataViewHolder(
        private val binding: ItemSignsBinding
    ): RecyclerView.ViewHolder(binding.root) {
        fun bind(signModel: SignModel) {
            Picasso.get().load(signModel.url).into(binding.signImg)
            binding.imgTitle.text = signModel.title
            binding.imgDescription.text = signModel.description
        }
    }
